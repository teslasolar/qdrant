...
with correct navigation based on what files actually exist
"""
import os
...
def generate_hmi_html(area_name, area_path_from_os, has_index, has_readme):
    ...
    # Build navigation buttons based on what files exist
    nav_buttons = []

    if has_index:
        nav_buttons.append('''    <div class="control-btn" onclick="window.location.href='index.html'">
      ...
      <div class="control-status">Index Page</div>
        ...
        nav_buttons.append('''    <div class="control-btn" onclick="window.open('README.md')">
      <div class="indicator"></div>
      <div class="control-icon">📖</div>
      <div class="control-label">README</div>
      <div class="control-status">Documentation</div>
    </div>''')

    nav_buttons.append('''    <div class="control-btn" onclick="window.location.href='scada.html'">
      <div class="indicator"></div>
      <div class="control-icon">📈</div>
    ...
    nav_buttons.append('''    <div class="control-btn" onclick="window.location.href='plc.html'">
      <div class="indicator"></div>
...
def generate_plc_html(area_name, area_path_from_os, has_index, has_readme):
    ...
    # Build breadcrumb
    breadcrumb_parts = ["<a href=\"{root_path}index.html\">OS Home</a>".format(root_path=root_path)]
    if has_index:
        breadcrumb_parts.append(f"<a href=\"index.html\">{title}</a>")
    else:
        breadcrumb_parts.append(f"<span style=\"color: #00ff88;\">{title}</span>")
    breadcrumb_parts.append("PLC")
    breadcrumb = " / ".join(breadcrumb_parts)

    # Build view buttons
    view_buttons = []
    if has_index:
        view_buttons.append('<button class="btn" onclick="window.location.href=\'index.html\'">View Main</button>')
    elif has_readme:
        view_buttons.append('<button class="btn" onclick="window.open(\'README.md\')">View README</button>')
    view_buttons.append('<button class="btn secondary" onclick="window.location.href=\'hmi.html\'">HMI Panel</button>')
    view_buttons.append('<button class="btn secondary" onclick="window.location.href=\'scada.html\'">SCADA View</button>')
    view_buttons_html = '\n      '.join(view_buttons)
...
def generate_scada_html(area_name, area_path_from_os, has_index, has_readme):
    ...
    sidebar_nav = []
    if has_index:
        sidebar_nav.append('<button class="btn" style="width: 100%; margin-bottom: 10px;" onclick="window.location.href=\'index.html\'">📄 Main View</button>')
    elif has_readme:
        sidebar_nav.append('<button class="btn" style="width: 100%; margin-bottom: 10px;" onclick="window.open(\'README.md\')">📖 README</button>')

    sidebar_nav.append('<button class="btn secondary" style="width: 100%; margin-bottom: 10px;" onclick="window.location.href=\'hmi.html\'">🎛️ HMI Panel</button>')
    sidebar_nav.append('<button class="btn secondary" style="width: 100%; margin-bottom: 10px;" onclick="window.location.href=\'plc.html\'">⚙️ PLC Logic</button>')
    sidebar_nav_html = '\n      '.join(sidebar_nav)
...
def main():
    ...
    for root, dirs, files in os.walk(OS_DIR):
        ...
        has_index = (root_path / "index.html").exists()
        ...
        # Only update if we already created interface files
        if has_hmi or has_plc or has_scada:
            # Update HMI file
            hmi_path = root_path / "hmi.html"
            hmi_path.write_text(generate_hmi_html(area_name, str(area_path_from_os), has_index, has_readme))
            updated_files.append(str(hmi_path))
            print(f"Updated: {hmi_path}")

            # Update PLC file
            plc_path = root_path / "plc.html"
            plc_path.write_text(generate_plc_html(area_name, str(area_path_from_os), has_index, has_readme))
            updated_files.append(str(plc_path))
            print(f"Updated: {plc_path}")

            # Update SCADA file
            scada_path = root_path / "scada.html"
            scada_path.write_text(generate_scada_html(area_name, str(area_path_from_os), has_index, has_readme))
            updated_files.append(str(scada_path))
            print(f"Updated: {scada_path}")
